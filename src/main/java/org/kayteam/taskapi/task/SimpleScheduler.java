...
public class SimpleScheduler {
    ...
    public void runSchedulerTimer(boolean async) {
        ...
        if ( bukkitTask != null )   stopScheduler();

        else

            if ( async )   bukkitTask = javaPlugin.getServer().getScheduler().runTaskTimerAsynchronously(javaPlugin, this::actions, 0, ticks);

            else   bukkitTask = javaPlugin.getServer().getScheduler().runTaskTimer(javaPlugin, this::actions, 0, ticks);
    ...
    public void runSchedulerLater(boolean async) {
        ...
        if (bukkitTask != null) stopScheduler();
             ...
             if ( async )   bukkitTask = javaPlugin.getServer().getScheduler().runTaskLaterAsynchronously(javaPlugin, this::actions, ticks);

             else bukkitTask = javaPlugin.getServer().getScheduler().runTaskLater(javaPlugin, this::actions, ticks);
    ...
    private void actions() {
        ...
        if ( actions.isEmpty() )   stopScheduler();

        else   actions.removeIf(scheduleAction -> scheduleAction.action(bukkitTask));
